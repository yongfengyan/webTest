...
//YYF modify on 20190607
function DataAnalysis(data,timezone)
{
	var str='';
	var temp;
	var len=data.historyDate.length;
	if(timezone == null)
	{
		timezone = "+8";
	}
	...
	var tzSecond = 0;
	$.each(data.historyDate,function(i,ele){
		if(zoneOp =='+')
		{
			temp = Date.parse(ele.markTime)+tzSecond;
		}
		if(zoneOp =='-')
		{
			temp = Date.parse(ele.markTime)-tzSecond;
    ...
    function DataAnalysis_orignal(data,timezone)
    {
        var str='';
        var temp;
